...
export const canCreateRole = (actor: TeamActor, requestedRole: UserRole) =>
  actor.role === 'SUPER_ADMIN' || actor.role === 'DIRECTOR' || (actor.role === 'CST_MANAGER' && requestedRole === 'CST_HANDLER');

export const canManageMember = (actor: TeamActor, member: TeamMember) =>
  actor.role === 'SUPER_ADMIN' || actor.role === 'DIRECTOR' || isOwnedHandler(actor, member);
...
export const canReassignClient = (actor: TeamActor, target: TeamMember, current?: TeamMember | null) => {
  ...
  if (actor.role === 'SUPER_ADMIN' || actor.role === 'DIRECTOR') return true;
