...
import { CheckCircle2, Download, Ellipsis, Filter, Pencil, RotateCcw, Trash2 } from "lucide-react";
...
export function OperationsView({ type, initialStatus = "All", initialFrom = "", initialTo = "" }: { type: OperationType; initialStatus?: string; initialFrom?: string; initialTo?: string }) {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => {
    const matchesQuery = `${row.client} ${row.detail} ${row.owner} ${row.notes}`.toLowerCase().includes(query.toLowerCase());
    return matchesQuery && (statusFilter === "All" || row.status === statusFilter) && (!from || row.dateIso >= from) && (!to || row.dateIso <= to);
  }), [from, query, rows, statusFilter, to]);
  ...
  const stats = useMemo(() => {
    ...
    if (type === "reports") return [["Total reports", rows.length], ["Sent", rows.filter((row) => row.status === "Sent").length], ["Pending", rows.filter((row) => row.status === "Pending").length], ["Late", rows.filter((row) => row.status === "Late").length]];
    if (type === "complaints") return [["Total logged", rows.length], ["Resolved", rows.filter((row) => row.status === "Resolved").length], ["Still open", rows.filter((row) => row.status === "Open").length], ["Resolution rate", `${rows.length ? Math.round(rows.filter((row) => row.status === "Resolved").length / rows.length * 100) : 0}%`]];
    return [["Pipeline", `$${rows.filter((row) => row.status === "In Progress").reduce((sum, row) => sum + (row.value ?? 0), 0).toLocaleString()}`], ["Converted", rows.filter((row) => row.status === "Converted").length], ["Revenue", `$${rows.filter((row) => row.status === "Converted").reduce((sum, row) => sum + (row.value ?? 0), 0).toLocaleString()}`], ["Lost", rows.filter((row) => row.status === "Lost").length]];
  }, [rows, type, visibleRows.length]);
  ...
  return <><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} action={<AddButton onClick={openAdd} testId={`add-${config.kind.toLowerCase()}`}>{config.add}</AddButton>} secondary={<Button variant="secondary" onClick={exportCsv}><Download size={15} />Export CSV</Button>} />
    ...
    <section className="panel" data-testid={`${config.kind.toLowerCase()}-table`}>
      <div className="toolbar"><div className="toolbar-group"><SearchField value={query} onChange={setQuery} placeholder={`Search ${config.title.toLowerCase()}…`} /><Button variant="secondary" onClick={() => setFiltersOpen((value) => !value)}><Filter size={15} />Filters</Button></div><Button variant="ghost" onClick={() => { setFrom(""); setTo(""); setStatusFilter("All"); }}><RotateCcw size={15} />Reset</Button></div>
      {filtersOpen && <div className="operation-filters"><Field label="Status"><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option>All</option>{statusOptions[type].map((status) => <option key={status}>{status}</option>)}</select></Field><Field label="From"><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></Field><Field label="To"><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></Field></div>}
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Client</th><th>{config.kind === "Contact" ? "Channel" : config.kind === "Upsell" ? "Service pitched" : "Record"}</th><th>Date</th><th>Owner</th>{config.kind === "Upsell" && <th>Revenue</th>}<th>Status</th><th /></tr></thead><tbody>{visibleRows.map((row) => <tr key={row.id}><td><strong>{row.client}</strong></td><td>{row.detail}</td><td>{row.date}</td><td>{row.owner}</td>{config.kind === "Upsell" && <td><strong>${row.value?.toLocaleString()}</strong></td>}<td><Badge tone={tone(row.status)}>{row.status}</Badge></td><td><button className="table-action" onClick={() => openDetail(row)} aria-label={`Open ${config.kind.toLowerCase()} for ${row.client}`}><Ellipsis size={16} /></button></td></tr>)}</tbody></table>{visibleRows.length === 0 && <div className="inline-empty">No matching records</div>}</div>
    </section>
